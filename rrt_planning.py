# Written by Caelan Garrett, modified a bit by Tomas Lozano-Perez, minor edits for ease of use in class project by Julianna Schneider
# yapf: disable
from manipulation.exercises.trajectories.rrt_planner.geometry import Point, Line, Polygon, Object, AABB, convex_hull
...
class Problem:
    ...
    def valid_configuration(self, configuration):
        ...
        """
        valid_cspace = self.cspace.valid_configuration(configuration)
        collides = self.collide(configuration)

        print(f"Configuration: {configuration}")
        print(f" - Valid in C-space: {valid_cspace}")
        print(f" - Collision detected: {collides}")

        return valid_cspace and not collides
    ...
    def collide(self, configuration):  # Check collision for configuration
        ...
        Returns:
            ...
            False otherwise
        """
        print("What happpens with collide")
    ...
    def safe_path(self, start, end):  # returns subset of path that is safe
        ...
        for i, configuration in enumerate(path):
            ...
            if i == 0:
                continue
            if self.collide(configuration):
                print("             Len Safe path:", len(safe_path))
                return safe_path
