...
def matching_q(q_actual, q_des, atol=1e-8):
...
def matching_q_holding_obj(q_actual, q_des, atol=1e-8):
    res = np.isclose(q_actual, q_des, atol=atol)
...
class RRT:
    ...
    def nearest(self, configuration):
        ...
        nearest_node = recur(self.root)[0]
        # self.visited_nodes.append(nearest_node.value)
        return nearest_node
...
class RRT_tools:
    ...
    def find_nearest_node_in_RRT_graph(self, q_sample):
        ...
        print("Nearest node: ", nearest_node.value)
        return nearest_node
